...
func main() {
	...
	qqclient := client.NewClient(uint32(3389583001), appInfo, "https://sign.lagrangecore.org/api/sign/25765")
	...
	qqclient.PrivateMessageEvent.Subscribe(func(client *client.QQClient, event *message.PrivateMessage) {
		_, err := client.SendPrivateMessage(event.Sender.Uin, []message.IMessageElement{message.NewText(event.Sender.Nickname)})
		if err != nil {
			return
		}
	})
